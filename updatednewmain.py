...
#####################################################################
# 7.5 Diagnosis Parsing & Matching Helpers (NEW)
#####################################################################

# Regex & helpers
_RE_DX_LINE = re.compile(r"^\s*dx\s*[:\-]\s*(.+)$", re.IGNORECASE | re.MULTILINE)
_RE_MOST_LIKELY = re.compile(r"(?:most\s+likely|likely|primary)\s+diagnosis\s*(?:is|:)\s*([^.\n;]+)", re.IGNORECASE)
...
def _aliases(label: str) -> set:
    """
    Build alias set from various simple transformations:
    ...
    # e.g., "disease", "syndrome", "tumor", "carcinoma" sometimes added/omitted
...
def _extract_predicted_dx(output: str) -> str:
    """
    Prefer a leading 'DX: ...' line; otherwise try 'most likely diagnosis is ...';
    otherwise fallback to the first line/sentence.
    """
    if not output:
        return ""
    m = _RE_DX_LINE.search(output)
    if m:
        return m.group(1).strip()
    m = _RE_MOST_LIKELY.search(output)
    if m:
        return m.group(1).strip()
...
def call_model_api(rewritten: str, model_identifier: str) -> str:
    ...
    if model_identifier == "MedAlpaca-7B-LBB":
        ...
        api_url = model_identifier  # model_identifier is the endpoint URL
        full_prompt = (
        ...
        if toks.input_ids.shape[1] > 512:
            toks_ids = toks.input_ids[:, :512]
            full_prompt = tokenizer.decode(toks_ids[0], skip_special_tokens=True)
...
def evaluate_prediction(output: str, true_dx: str, distractors: List[Optional[str]]) -> Dict[str, Any]:
    ...
    # 4) Fuzzy match thresholds (handle small lexical variants)
    #    First against TRUE
    ...
    # If the predicted label doesn't match aliases, but the output text mentions the true dx,
    # treat as ambiguous (model may have explained context while naming something else).
    ...
    # 6) If it matches none of the above and mentions a distractor elsewhere, call it 'fooled_other'
    ...
    # 7) Unknown (could not confidently adjudicate)
...
def main():
    ...
    SELECTED_MODEL = "BioMistral-7B-UXI"  # e.g., "GPT-4", "ChatDoctor-AEA", etc.
    ...
    # Subset of cases for evaluation (e.g., first 7 cases)
    subset = ds.select(range(7))
    results = []
    ...
    # Save results to a JSON file (kept same filename for compatibility)
    out_file = "evaluation_results_med_alpaca_7b_lbb45.json"
